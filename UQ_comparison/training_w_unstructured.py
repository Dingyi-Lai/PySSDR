...
def predict_effects(scenario_index_folder, read_path, save_path, case, data, train_parameters,
                    num_knots, grid_size, output_dimension_dnn, true_nonlinear_effects, add_unstructured=True, 
                    modify=True, ortho_manual=False):
    ...
    item_unstructured = ""
    if add_unstructured:
        item_unstructured = " + dnn(Image)"

    if distribution == "poisson":
        distribution_SSDR = "Poisson" # compatible form
        formulas = {'rate': f"~ 1 + X1 + X2 + spline(Z1, bs='bs', df={num_knots+4}) + spline(Z2, bs='bs', df={num_knots+4})"+item_unstructured}
        degrees_of_freedom = {'rate':num_knots+4}
    elif distribution == "gamma":
        distribution_SSDR = "Gamma" # compatible form
        formulas = {
        'loc': f"~ 1 + X1 + X2 + spline(Z1, bs='bs', df={num_knots+4}) + spline(Z2, bs='bs', df={num_knots+4})"+item_unstructured,
        'scale': '~ 1'
        }
        ...
        formulas = {
        'loc': f"~ 1 + X1 + X2 + spline(Z1, bs='bs', df={num_knots+4}) + spline(Z2, bs='bs', df={num_knots+4})"+item_unstructured,
        'scale': f"~ 1 + X1 + X2 + spline(Z1, bs='bs', df={num_knots+4}) + spline(Z2, bs='bs', df={num_knots+4})"+item_unstructured,
        }
        degrees_of_freedom = {'loc':num_knots+4, 'scale':num_knots+4}
...
def training_task(n_sample, distribution_list, SNR_list, method_list, grid_size, train_parameters, 
                  num_knots, n_rep, read_path, save_path, compute_type='parallel', add_unstructured=True,
                  modify=True, ortho_manual=False):
    ...
    if compute_type == 'parallel':
        def process_combination(c):
            df_results = predict_effects(scenario_index, read_path, save_path, c, df, train_parameters,
                                             ...
                                             add_unstructured, modify, ortho_manual)
            # key = f"n_{n_sample}_rep_{n_rep}_dist_{c[0]}_SNR_{c[1]}_method_{c[2]}"
            return df_results

        with ThreadPoolExecutor() as executor:
            df_results = executor.map(process_combination, combinations)
...
def uq_comparison(n_list, distribution_list, SNR_list, method_list, grid_size,
                  ...
                  compute_type='parallel', add_unstructured=True, modify=True, ortho_manual=False):
        ...
        with mp.Pool(n_cores) as pool:
            results = pool.starmap(
                ...
                [(i, distribution_list, SNR_list, method_list, grid_size, train_parameters_list[n_list.index(i)], 
                  num_knots, r, read_path, save_path, compute_type, add_unstructured, modify, ortho_manual)
                 for (i, r) in replicates]
            )
    elif compute_type == 'serial':
        for (i, r) in replicates:
            results = training_task(n_sample=i, distribution_list=distribution_list, 
